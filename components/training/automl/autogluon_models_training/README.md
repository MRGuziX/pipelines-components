...
| `positive_class` | `Optional[str]` | `None` | Optional label value for the positive class in **binary** classification. Passed to ``TabularPredictor`` when set. If ``None`` or empty, AutoGluon infers it. Ignored for ``multiclass`` and ``regression``. |
